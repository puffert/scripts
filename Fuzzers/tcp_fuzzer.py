...
def tcp_fuzzer(ip, port, prefix, interval, timeout):
    payload_length = 0
    ...
    while True:
        try:
            ...
            packet = IP(dst=ip) / fuzz(TCP(dport=port) / Raw(load=payload))
...
def main():
    parser = argparse.ArgumentParser(description="TCP Fuzzer")
    ...
    tcp_fuzzer(args.ip, args.port, args.prefix, args.interval, args.timeout)
